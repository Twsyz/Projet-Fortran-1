...
SUBROUTINE Time_Step(recup, dim, get, time, x_reg, y_reg, dt, Kappa)
    USE m_type
    IMPLICIT NONE
...
SUBROUTINE DIFFUSION(recup, get, dim, conc, flux_d, Kappa)
    USE m_type 
    IMPLICIT NONE 
    TYPE(phys), INTENT(IN) :: recup
    TYPE(num), INTENT(IN) :: get
    ...
    flux_d%Q1 = -Kappa * (conc%CE - conc%CA) / dx ! Flux en j de (i-1) -> i OUEST
    flux_d%Q2 = -Kappa * (conc%CB - conc%CE) / dx ! Flux en j de i -> (i+1) EST
    flux_d%Q3 = -Kappa * (conc%CE - conc%CC) / dy ! Flux en i de (j-1) -> j SUD
    flux_d%Q4 = -Kappa * (conc%CD - conc%CE) / dy ! Flux en i de j -> (j+1) NORD
...
SUBROUTINE STEP_TRANSPORT(recup, get, dim, time, C, C_new, x_reg, y_reg)
    ...
    DO j = 2, get%Ny-1
        DO i = 2, get%Nx-1
            ...
            ! Diffusion
            CALL DIFFUSION(recup, get, dim, conc, flux_d, recup%K)
...
FUNCTION max_concentration(get, C)

    USE m_type
    IMPLICIT NONE

    TYPE(num), INTENT(IN) :: get
    REAL, DIMENSION(get%Nx,get%Ny), INTENT(IN) :: C

    REAL, DIMENSION(3) :: max_concentration
    REAL :: cmax
    INTEGER :: i, j
    INTEGER :: imax, jmax
    ...
    DO j = 1, get%Ny
        ...
        END DO
    END DO
    max_concentration(1) = cmax
    max_concentration(2) = REAL(imax)
    max_concentration(3) = REAL(jmax)

END FUNCTION max_concentration
