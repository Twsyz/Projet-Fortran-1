...
SUBROUTINE mesh(x_reg, y_reg, dim, get, Nxx, Nyy)
    USE m_type
    IMPLICIT NONE
    ...
    INTEGER :: Nxx, Nyy
    REAL, DIMENSION(Nxx), INTENT(OUT) :: x_reg
    REAL, DIMENSION(Nyy), INTENT(OUT) :: y_reg
    INTEGER :: i,j
    REAL :: dx, dy

    dx = 2.0*dim%L / REAL(Nxx)
    dy = dim%L / REAL(Nyy)
...
FUNCTION Cinit(recup, get, dim, x, y, Nxx, Nyy)
	USE m_type 
	IMPLICIT NONE 
    ...
    DELTA = (2./3.)*min((2*dim%L)/real(Nxx), (dim%L)/real(Nyy))
...
FUNCTION diff_concentration(get, Cnew, Cold)
    ...
    TYPE(num), INTENT(IN) :: get
    REAL, DIMENSION(get%Nx,get%Ny), INTENT(IN) :: Cnew, Cold
