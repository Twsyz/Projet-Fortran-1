...
SUBROUTINE Vitesse(recup, dim, x, y, V, alpha1)
    USE m_type
    IMPLICIT NONE
    ...
    V(1) =  alpha1 * SIN(pi*x/dim%L) * COS(pi*y/dim%L)
    V(2) = -alpha1 * COS(pi*x/dim%L) * SIN(pi*y/dim%L)
...
SUBROUTINE Champ_Vitesse(recup, dim, get, x_reg, y_reg, U)
    ...
    DO i=1,get%Nx
        DO j=1,get%Ny
            CALL Vitesse(recup, dim, x_reg(i), y_reg(j), V, recup%alpha)
            U(i,j,:) = V
        END DO
    END DO
...
SUBROUTINE Champ_Vitesse_Faces(recup, dim, get, x_noeud, y_noeud, U_face, V_face)
    ...
    DO j = 1, get%Ny
        DO i = 1, get%Nx+1
            x_face = x_noeud(i, j)
            y_face = y_noeud(i, j)
            CALL Vitesse(recup, dim, x_face, y_face, V, recup%alpha)
...
SUBROUTINE ADVECTION(recup, dim, conc, x, y, flux_a)
    ...
    CALL Vitesse(recup, dim, x, y, V, recup%alpha)
