    ...
    DO i=1,get%Nx
        DO j=1,get%Ny
            ...
            dt_local = 1.0 / (ABS(V(1))/(dx*time%CFL) + &
                              ABS(V(2))/(dy*time%CFL) + & 
                              (recup%K/(time%R*dx*dx)) + & 
                              (recup%K/(time%R*dy*dy)))
...
! -------------------------------------------------------------------------------
! Solution analytique pour diffusion 1D en x
! Adaptée pour la direction x
! -------------------------------------------------------------------------------
FUNCTION analytical_solution_x(x, t, recup, dim)
    USE m_type
    IMPLICIT NONE
    ...
    TYPE(taille), INTENT(IN) :: dim
    REAL, INTENT(IN) :: x, t
    REAL :: analytical_solution_x
    REAL :: arg
    
    ! Pour x, le domaine va de 0 à 2L, donc le centre est à x = L
    IF (t > 0.0) THEN
        arg = (x - dim%L) / (2.0 * SQRT(recup%K * t))
    ELSE
        ! Condition initiale
        IF (x < dim%L) THEN
            analytical_solution_x = recup%C1
        ELSE
            analytical_solution_x = recup%C0
        END IF
        RETURN
    END IF
    
    analytical_solution_x = recup%C0 + (recup%C1 - recup%C0)/2.0 * (1.0 - ERF(arg))
    
END FUNCTION analytical_solution_x
