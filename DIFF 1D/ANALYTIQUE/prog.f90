! cd /c/Users/moi/Desktop/buro/N7/Methodes\ Volumes\ Finis/BE/DIFF\1D/X
PROGRAM main_total
    USE m_type
    IMPLICIT NONE
    ...
    REAL :: Cinit,MIN_VALUE, analytical_solution_x

    INTEGER :: i, j, n, i_midie
    REAL :: t, dt 
    ...
    DO WHILE (t < time%tf .AND. n < get%Nt )
        ...
        ! --- Sauvegarde à chaque temps en x=L/2 ---
        WRITE(20,*) "TIME", t

        i_midie = get%Nx/2
        DO i = 1, get%Nx
            WRITE(20,*) x_reg(i), C(i,get%Ny/2), analytical_solution_x(x_reg(i), t, recup, dim)
        END DO

        IF(MOD(n, 100) == 0) THEN
                PRINT *, 'Pas de temps :', n, 'Temps :', t
            END IF
